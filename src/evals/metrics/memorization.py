...
def eval_text_similarity_batch(model, tokenizer, batch, generation_args):
    ...
    output = model.generate(input_ids, attention_mask=attention_mask, **generation_args, pad_token_id=tokenizer.eos_token_id)
    gen_text = tokenizer.batch_decode(output[:, input_ids.shape[-1]:], skip_special_tokens=True)
...
def eval_text_similarity(model, tokenizer, dataloader, generation_args):
        index_to_scores = {}
        for batch in tqdm(dataloader, desc="Calculating Text Similarity", total=len(dataloader)):
...
def qa_prob(model, **kwargs):
    ...
    dataloader = DataLoader(
            data, batch_size=batch_size, collate_fn=collator
        )
...
data_cfg = DictConfig({
    "TOFU_QA_FORGET10_PT": {
        "args": {
            "hf_args":{
                "data_files": "./data/TOFU_FORGET10_PERTURBED.json",
                "split": "train",
                "path": "json",
            },
            "question_key": "question",
...
def qa_rouge(model, **kwargs):
    ...
    index_to_scores = eval_text_similarity(model, tokenizer, dataloader, generation_args)
    return index_to_scores
...
@unlearning_metric(name='TOFU_QA_PT_ROUGE', data_cfg=data_cfg, collator_cfg=collator_cfg)
...
@unlearning_metric(name='TOFU_QA_BIO_Prob', data_cfg=data_cfg, collator_cfg=collator_cfg)
...
@unlearning_metric(name='TOFU_QA_BIO_ROUGE', data_cfg=data_cfg, collator_cfg=collator_cfg)
